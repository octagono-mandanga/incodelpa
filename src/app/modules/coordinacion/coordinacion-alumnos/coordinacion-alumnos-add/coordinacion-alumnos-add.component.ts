...
@Component({
  selector: 'app-coordinacion-alumnos-add',
  templateUrl: './coordinacion-alumnos-add.component.html',
  styleUrls: ['./coordinacion-alumnos-add.component.css']
})
export class CoordinacionAlumnosAddComponent implements OnInit {
  public config: any
  ...
  onAgregar(data: any){
    data.rol = this.id;
    this.loading = true
    this.crudService.create(data, '/coordinacion/alumnos').subscribe({
      next: (res: any) => {
       this.data = new User
