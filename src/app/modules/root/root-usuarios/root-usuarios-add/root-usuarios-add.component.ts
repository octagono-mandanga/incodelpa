...
@Component({
  selector: 'app-root-usuarios-add',
  templateUrl: './root-usuarios-add.component.html',
  styleUrls: ['./root-usuarios-add.component.css']
})
export class RootUsuariosAddComponent implements OnInit {
  public config: any
  ...
  async ngOnInit() {
    ...
    const data: TipoDocumento[] = await firstValueFrom(this.crudTipodocumentoService.read('/root/tipo_documentos'));
  ...
  onAgregar(data: any){
    data.rol = this.id;
    this.loading = true
    this.crudService.create(data, '/root/usuarios').subscribe({
      next: (res: any) => {
       this.data = new User
