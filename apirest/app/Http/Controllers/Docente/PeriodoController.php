    ...
    public function index()
    {
        ...
        $periodos = Periodo::
            whereHas('lectivo', function ($query) {
                $query->where('lectivos.estado', 'activo');
            })
            ->join('lectivos', 'lectivos.id', '=', 'periodos.lectivo')
            ->join('niveles', 'niveles.id', '=', 'lectivos.nivel')
            ->orderBy('niveles.orden')
            ->orderBy('periodos.created_at')
            ->select('periodos.*') // Asegúrate de seleccionar los campos necesarios, evitando conflictos de nombres de columnas.
            ->with(['lectivo.nivel'])
            ->get();
