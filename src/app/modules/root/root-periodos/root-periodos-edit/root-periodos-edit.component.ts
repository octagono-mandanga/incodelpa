...
@Component({
  selector: 'app-root-periodos-edit',
  templateUrl: './root-periodos-edit.component.html',
  styleUrls: ['./root-periodos-edit.component.css']
})
export class RootPeriodosEditComponent implements OnInit {
  public config: any
  public id!: string
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      ...
      this.crudService.read('/root/periodos/'+this.id).subscribe({
        next: (res: any) => {
          this.data = res.data
  ...
  onEditar(data: any){
    this.loading = true
    this.crudService.update(data, '/root/periodos/'+this.id).subscribe({
      next: (response: any) => {
