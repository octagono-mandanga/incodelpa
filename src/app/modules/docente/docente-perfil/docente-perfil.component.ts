...
export class DocentePerfilComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    const data: TipoDocumento[] = await firstValueFrom(this.crudTipodocumentoService.read('/docente/tipo_documentos'));
    ...
    await this.crudService.read('/docente/auth').subscribe({
      next: (res: any) => {
        this.data = res.data
        this.loading = false
       },
  ...
  onActualizar(event: any){
    ...
    this.loading = true
    this.crudService.update(event, '/docente/auth/update').subscribe({
      next: (res: any) => {
        this.data = res.data
