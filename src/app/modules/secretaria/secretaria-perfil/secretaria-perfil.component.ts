...
@Component({
  selector: 'app-secretaria-perfil',
  templateUrl: './secretaria-perfil.component.html',
  styleUrls: ['./secretaria-perfil.component.css']
})
export class SecretariaPerfilComponent implements OnInit {
  public config: any
  ...
  async ngOnInit() {
    ...
    await this.crudService.read('/secretaria/auth').subscribe({
      next: (res: any) => {
        this.data = res.data
        this.loading = false
       },
  ...
  onActualizar(event: any){
    ...
    this.loading = true
    this.crudService.update(event, '/secretaria/auth/update').subscribe({
      next: (res: any) => {
        this.data = res.data
